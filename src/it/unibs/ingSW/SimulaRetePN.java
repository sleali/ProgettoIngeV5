...
public class SimulaRetePN {
	...
	public void scegli() {
		...
		if(names.length > 0)
		{
			...
			rete.carica(DIRECTORYPN + names[descr - 1], ElementoPN.class);
			updateRete();
		}
		else 
	...
	private void updateRete() {
		...
		for(Elemento el : rete.getElementi()) {
			...
			for(PostoPN postoPN : posti) {
				if(postoPN.isEqual(((ElementoPN)el).getPosto()))
					equalPosto=true;
			}
			for(TransizioneN transizionePN : transizioni) {
				if(transizionePN.isEqual(((ElementoPN)el).getTransizione()))
					equalTransizione=true;
			}
			if(!equalPosto)
				posti.add(((ElementoPN)el).getPosto());
			if(!equalTransizione)
				transizioni.add(((ElementoPN)el).getTransizione());
		}
	}
	...
	public ArrayList<TransizioneN> transizioniAbilitate() {
		...
		for(TransizioneN trans : transizioni) {
			boolean abil=true;
			for(Elemento el : rete.getElementi()) {
				if(el.getVerso() && trans.getID()==el.getIdTransizione()){
					int marcatura = ((ElementoPN)el).getPosto().getMarcatura();
					int peso = ((ElementoPN)el).getPeso();
					if(marcatura-peso<0) {
						abil=false;
					}
				}
	...
	public ArrayList<PostoPN> predecessori(TransizioneN trans){
		...
		for(Elemento elem : rete.getElementi()) {
			if(elem.getIdTransizione() == trans.getID() && elem.getVerso()) {
				pred.add(((ElementoPN)elem).getPosto());
			}
		}
		return pred;
	}
	...
	public ArrayList<PostoPN> successori(TransizioneN trans){
		...
		for(Elemento elem : rete.getElementi()) {
			if(elem.getIdTransizione() == trans.getID() && !elem.getVerso()) {
				succ.add(((ElementoPN)elem).getPosto());
			}
		}
		return succ;
	}
	...
	public void scattaTransizione(TransizioneN trans) {
		...
		for(PostoPN posto : scatto1) {
			...
			for(Elemento el : rete.getElementi()) {
				ElementoPN elNew = (ElementoPN)el;
